export default {
  async fetch(request) {
    ...
    if (url.pathname === "/api/stats") {
      return new Response(JSON.stringify({totalEvents:12847,activeDays:23,appsTracked:156,sessions:89}), {headers:{"Content-Type":"application/json"}});
    }
    if (url.pathname === "/api/events") {
      return new Response(JSON.stringify([
        {id:1,kind:"window",app:"VS Code",title:"working on catchme.py",timestamp:Date.now()-120000},
        {id:2,kind:"keystroke",app:"Terminal",title:"git commit",timestamp:Date.now()-300000},
        {id:3,kind:"clipboard",app:"Chrome",title:"copied code",timestamp:Date.now()-480000}
...
'</div>' +
'<script>' +
'async function loadStats(){try{const res=await fetch("/api/stats");const data=await res.json();document.getElementById("totalEvents").textContent=data.totalEvents.toLocaleString();document.getElementById("activeDays").textContent=data.activeDays;document.getElementById("appsTracked").textContent=data.appsTracked;document.getElementById("sessions").textContent=data.sessions;}catch(e){console.error(e)}}' +
'async function loadEvents(){try{const res=await fetch("/api/events");const events=await res.json();const list=document.getElementById("eventsList");var html="";for(var i=0;i<events.length;i++){var e=events[i];var time=Math.round((Date.now()-e.timestamp)/60000);html=html+\'<div class="event"><span class="event-app">\'+e.app+\'</span><span class="event-time">\'+time+\' min ago</span><div class="event-title">\'+e.title+\'</div></div>\';}list.innerHTML=html;}catch(e){console.error(e)}}' +
